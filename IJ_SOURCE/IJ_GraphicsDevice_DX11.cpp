...
namespace IJ::graphics
{
    ...
    void GraphicsDevice_DX11::SetConstantBuffer(eShaderStage stage, eCBType type, ID3D11Buffer* buffer)
    {
        switch (stage)
        {
        case IJ::graphics::eShaderStage::VS:
        {
            mContext->VSSetConstantBuffers((UINT)type, 1, &buffer);
        }
        break;
        case IJ::graphics::eShaderStage::HS:
        {
            mContext->HSSetConstantBuffers((UINT)type, 1, &buffer);
        }
        break;
        case IJ::graphics::eShaderStage::DS:
        {
            mContext->DSSetConstantBuffers((UINT)type, 1, &buffer);
        }
        break;
        case IJ::graphics::eShaderStage::GS:
        {
            mContext->GSSetConstantBuffers((UINT)type, 1, &buffer);
        }
        break;
        case IJ::graphics::eShaderStage::PS:
        {
            mContext->PSSetConstantBuffers((UINT)type, 1, &buffer);
        }
        break;
        case IJ::graphics::eShaderStage::CS:
        {
